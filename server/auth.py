...
from server.db import User, ApiKey, utcnow
...
def bind_channel_identity(
    ...
    """
    from server.db import UserIdentity, utcnow
...
def store_drive_token(session: Session, user_id: int, token_json: str) -> 'UserDriveToken':
    ...
    from server.db import UserDriveToken
...
def get_drive_service_account_email() -> str | None:
    ...
    from tools.tool_drive import _load_service_account_creds
...
def drive_connection_status(session: Session, user_id: int) -> dict:
    ...
    """
    from tools.tool_drive import has_service_account
