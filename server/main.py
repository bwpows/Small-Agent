...
from server.db import init_db, get_session, ApiKey, Conversation, User, UserIdentity
...
from server.limiter import get_limiter
...
from server.config import SERVER_BASE_URL
...
def list_tools(ctx: TenantContext = Depends(get_tenant_context)):
    """列出可用工具（MCP 格式）"""
    from core.llm_engine import get_tools_as_mcp
...
async def feishu_webhook(request: Request, session=Depends(get_db)):
    ...
    """
    from server.channels.feishu import (
...
async def feishu_login():
    ...
    """
    from server.config import FEISHU_APP_ID
...
async def feishu_callback(
    ...
    """
    from server.channels.feishu import exchange_oauth_code
...
def upload_service_account(
    ...
    from tools.tool_drive import set_service_account_from_json
...
def remove_service_account():
    ...
    from tools.tool_drive import clear_service_account
...
def drive_status(
    ...
    from server.auth import drive_connection_status
...
def google_login(user_id: int = Depends(jwt_user_id)):
    ...
    """
    from server.auth import google_oauth_url
...
def google_callback(
    ...
    [已废弃] Google OAuth 回调。
    """
    from server.auth import exchange_google_code, store_drive_token
...
def drive_revoke(
    ...
    """断开个人 Drive OAuth 授权"""
    from server.auth import revoke_drive_token
