...
class AssetRegistry(ABC):
    ...
    def sync_from_drive(self, limit: int = 50) -> List[BusinessAsset]:
        ...
        子类可选实现，默认空列表。"""
        try:
            from tools.tool_drive import authenticate_drive
...
class LocalJsonRegistry(AssetRegistry):
    ...
    def __init__(self, file_path: str = None):
        if file_path is None:
            ...
            file_path = os.path.join(project_root, "config", "business_assets.json")
...
class Mem0Registry(AssetRegistry):
    ...
    def _ensure_loaded(self):
        if self._loaded:
            return
        try:
            from tools.tool_memory import MemoryManager
