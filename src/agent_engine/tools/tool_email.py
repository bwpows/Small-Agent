...
from agent_engine import config as engine_config
...
def send_notification_email(subject, content, receiver_email):
    ...
    try:
        # 使用 465 端口和 SSL 加密
        server = smtplib.SMTP_SSL(engine_config.SMTP_SERVER, engine_config.SMTP_PORT, timeout=10)
        
        # 🌟 绝招 2：开启底层调试模式，把网络握手细节全部打印出来
        server.set_debuglevel(1)
        
        # 登录并发送
        server.login(engine_config.SENDER_EMAIL, engine_config.AUTH_CODE)
        server.sendmail(engine_config.SENDER_EMAIL, [receiver_email], message.as_string())
        server.quit()
        return "\n✅ 邮件发送成功！"
